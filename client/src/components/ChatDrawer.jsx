...
const unsafePatterns = [
    ...
    /\bawful\b/,
    /\bdumb\b/,
    /\b(moron|jerk|stfu|fuck|bitch|asshole|shit|damn)\b/,
    /\b(nude|naked|porn|sex|escort|erotic|xxx|adult)\b/,
    /\b(kill myself|suicide|hang myself|end my life|self harm)\b/,
    /\b(kill|murder|bomb|terrorist|shoot|knife|weapon|gun|shooting)\b/,
    /\b(drug|cocaine|heroin|meth|weed|opiate|fentanyl)\b/,
    /\b(password|credit card|ssn|social security|bank account|pin)\b/,
    /\b(how to hack|hack into|steal|fraud|illegal)\b/,
];
...
const getUnsafeResponse = () =>
    "I’m here to help with college questions. Please keep your messages respectful, and I cannot assist with that topic.";
...
export default function ChatDrawer({ open, onClose }) {
    ...
    const handleSend = async (text) => {
        ...
        if (isUnsafeMessage(messageText)) {
            const botMsg = { id: Date.now() + 1, text: getUnsafeResponse(), sender: "bot" };
